...
int main()
{
	...
	inputData.open("Customizable_Data.txt");
	...
	for (int z = 0; z < numPoints; z++){
		...
		for (int i = 0; i < numInput + z; i++){
			...
			distances2[i] = 0.0;
			for (int j = 0; j < numDim; j++){
				distances2[i] += (coors[j][i] - dataPoint[j])*(coors[j][i] - dataPoint[j]);
			}
		}
		...
		cudaMemcpy(devD, coors[0], (numInput + z)*sizeof(float), cudaMemcpyHostToDevice);
		...
		//Get the first dimension's distances
		distKernel << <blockSize, gridSize >> >(devD, dataPoint[0], numInput + z);

		for (int i = 1; i < numDim; i++){

			//Copy in the next dimension's data
			cudaMemcpy(devX, coors[i], (numInput + z)*sizeof(float), cudaMemcpyHostToDevice);
			
			//Compute the distances for the next dimension
			distKernel << <blockSize, gridSize >> >(devX, dataPoint[i], numInput + z);
			
			//Sum the squared differences using cuBlas
			cublasSaxpy(h, numInput + z, &alpha, devX, 1, devD, 1);
		...
		//Free the labelCount Cuda Array
		cudaFree(devL);
	...
	outputData.open("Customizable_Data_Updated.txt", ios::out | ios::trunc);
